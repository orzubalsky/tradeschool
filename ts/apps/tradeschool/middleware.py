...
class BranchMiddleware(object):
    def process_request(self, request):
        ...
        if branch_slug == None and request.method == 'POST' and request.is_ajax():

            # the incoming data from Dajaxice looks like a Python dictionary,
            # but it's actually a string. We're parsing it using the ast library
            ajax_dict = ast.literal_eval(request.POST.dict()['argv'])

            # after that we can get the branch_slug value like from any other dictionary
